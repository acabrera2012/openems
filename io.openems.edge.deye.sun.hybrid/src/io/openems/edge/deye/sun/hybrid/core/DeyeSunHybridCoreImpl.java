...
public class DeyeSunHybridCoreImpl extends AbstractOpenemsComponent
		implements DeyeSunHybridCore, OpenemsComponent, EventHandler {

	private final Logger log = LoggerFactory.getLogger(DeyeSunHybridCoreImpl.class);
	...
	public DeyeSunHybridCoreImpl() {
		super(//
				OpenemsComponent.ChannelId.values(), //
				DeyeSunHybridCore.ChannelId.values() //
		);
	}
	...
	private void initialize(Config config, InetAddress inverterAddress) throws Exception {
		 ...
		 * is no possibility to separate between the Deye versions before
		...
		if (inverterAddress != null) {
			 ...
			 */
			this.logInfo(this.log, "Deye core was configured with static ip: " + inverterAddress.getHostAddress());
		} else {
			/*
	...
	private void initClient(Config config, InetAddress inverterAddress) throws Exception {
		...
		InetAddress localAddress = DeyeSunHybridCoreImpl.getMatchingLocalInetAddress(inverterAddress);
	...
	/**
	 * Initialization of an older Deye version.
	 * 
	 * <p>
	...
	/**
	 * Initialization of a Deye version greater or equals 8.
	 * 
	 * @return boolean if there was a response of the initialization.
	 */
	private boolean initVersion8() {
		...
		if (this.accessBitTest(accessFeedb, 0) || accessFeedb == 0) {
			this.channel(DeyeSunHybridCore.ChannelId.MULTIPLE_ACCESS).setNextValue(false);
			return false;
		} else if (accessFeedb > 7) {
			// Deye is read by multiple device. Its response is invalid
			this.channel(DeyeSunHybridCore.ChannelId.MULTIPLE_ACCESS).setNextValue(true);
			return false;
		} else {
			this.channel(DeyeSunHybridCore.ChannelId.MULTIPLE_ACCESS).setNextValue(false);
			// Ident key accepted
