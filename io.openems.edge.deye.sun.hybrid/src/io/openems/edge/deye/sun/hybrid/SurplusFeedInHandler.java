...
public class SurplusFeedInHandler {
	...
	protected Integer run(List<DeyeSunHybridPv> chargers, Config config, ComponentManager componentManager) {
	 ...
	 * @return pv power
	 */
	private int getPvPower(List<DeyeSunHybridPv> chargers) {
		var pvPower = 0;
	...
	private boolean areSurplusConditionsMet(DeyeSunHybridImpl ess, List<DeyeSunHybridPv> chargers,
			Config config) {
		if (chargers.isEmpty()) {
			return false;
		}
		...
		var maxVoltage = 0;
		for (DeyeSunHybridPv charger : chargers) {
			int thisVoltage = ((IntegerReadChannel) charger
					.channel(DeyeSunHybridPv.ChannelId.PV_DCDC_INPUT_VOLTAGE)).value().orElse(0);
	...
	private void applyPvPowerLimit(List<DeyeSunHybridPv> chargers, Config config, boolean limitPv) {
		/*
