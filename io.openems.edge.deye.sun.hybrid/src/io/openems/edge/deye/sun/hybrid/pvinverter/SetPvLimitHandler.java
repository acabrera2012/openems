...
public class SetPvLimitHandler implements ThrowingRunnable<OpenemsNamedException> {
	...
	private final DeyeSunHybridPvInverterImpl parent;
	private final ChannelId channelId;

	private Float lastEpLimit = null;
	private LocalDateTime lastWMaxLimPctTime = LocalDateTime.MIN;

	public SetPvLimitHandler(DeyeSunHybridPvInverterImpl parent, ChannelId channelId) {
		this.parent = parent;
	...
	public void run() throws OpenemsNamedException {
		...
		if (powerOpt.isPresent()) {
			power = powerOpt.get();
			ePLimit = (int) ((double) power / DeyeSunHybridPvInverter.MAX_APPARENT_POWER * 100.0);
			...
			// Reset limit
			power = DeyeSunHybridPvInverter.MAX_APPARENT_POWER;
			ePLimit = 100;
		}
